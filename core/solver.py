...
class CaptioningSolver(object):
    def __init__(self, model, data, val_data, **kwargs):
        ...
        self.n_epochs = kwargs.pop('n_epochs', 1)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            with open('./resized_training_data/label_annotation.pkl', 'rb') as f:
                label_dict = pickle.load(f)
            new_label_dict =copy.deepcopy(label_dict)
            for k,v in label_dict.items():
                new_label_dict[k] = np.array(v, dtype=np.float32)

            with open('./resized_training_data/train/train.id_to_idx.pkl', 'rb') as f:
                id_to_idx = pickle.load(f)
            idx_to_id = dict((y,x) for x,y in id_to_idx.items())
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    batch_label = np.array([label_dict[idx_to_id[index]] for index in image_idxs_batch])

                    features_batch = features[image_idxs_batch]
                    feed_dict = {self.model.features: features_batch, self.model.captions: captions_batch, self.model.fc_labels: batch_label}
                ...
                if self.print_bleu:
                    ...
                    save_pickle(all_decoded, "./resized_training_data/val/val.candidate.captions.pkl")
    ...
    def test(self, data, split='train', attention_visualization=True, save_sampled_captions=True):
        ...
        with tf.Session(config=config) as sess:
            ...
            if save_sampled_captions:
                ...
                save_pickle(all_decoded, "./resized_training_data/%s/%s.candidate.captions.pkl" %(split,split))
